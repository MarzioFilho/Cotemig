							...
							<div class="col-sm-12 text-center">                
								<form name="formulario" method="POST" action="">
									<?php									
										...
										{
									?>
                                                                        <h1>Apagar Dietas</h1><br>
									<label class="text-danger">Qual dieta deseja apagar?</label>
									<br>
                                                                        <div id="bordas">
									<?php
											$verifica = "";
											...
											{
												$verifica = true;
												ECHO"<select name='dieta' class='form-control'>";
												while($row > 0)
												{
											...
											{
												$verifica = false;
												echo "Não há registros de dietas, Clique aqui para registrar: <a href='CadastraDieta.php'>Regstrar</a>";
											}
											...
											{	
												if($verifica === true)
												{
													...
													$sql2="delete from refeicao where cod_dieta=".$IndexCBX;
													$executeSQL = $conn->query($sql2);
													$sql = "delete from dieta where cod_dieta = ".$IndexCBX;
													if ($conn->query($sql) === TRUE) 
													{
														echo "<script>alert('Dieta removida com sucesso!');</script>";
														header('location: Exibe.php');
													}
													else 
													{
									...
									?>
									<br>
                                    <input type="submit" class="btn-danger btn-lg" name="sim" value="Apagar">
                                    <input type="submit" class="btn-warning btn-lg" name="nao" value="Voltar">	
									<?php
										}
										else
										{
